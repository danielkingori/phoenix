...
def test_topic_create(
    ...
    """Test create of the youtube_videos_topics run params."""
    ...
    data_set_name = "youtube_videos_topics"

    if rename_topic_to_class:
        data_set_name = "youtube_videos_classes"
    ...
    assert urls.input_dataset == f"{TAGGING_BASE}youtube_videos_final.parquet"
    assert urls.topics == f"{TAGGING_BASE}topics.parquet"
    if rename_topic_to_class:
        assert urls.tagging_final == f"{TAGGING_BASE}youtube_videos_classes_final.parquet"
    else:
