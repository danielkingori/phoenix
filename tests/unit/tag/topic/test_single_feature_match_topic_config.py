...
@mock.patch("phoenix.tag.topic.single_feature_match_topic_config._persist_topic_config_csv")
def test_commit_topic_config(m_persist):
    """Test the commit of topic config."""
    ...
    result_df = sfm_topic_config.commit_topic_config(topic_config)
    m_persist.assert_called_once_with(result_df, sfm_topic_config._default_config_url())
    pd.testing.assert_frame_equal(
        result_df,
        pd.DataFrame(
            {
