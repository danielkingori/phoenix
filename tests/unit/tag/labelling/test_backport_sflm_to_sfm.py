...
def test_sflm_to_sfm_use_processed_features():
    input_df = pd.DataFrame(
        data={
            "object_id": ["id_1", "id_2", "id_3", "id_4", "id_5"],
            "class": ["dog", "cat", "dog", "insect", "insect"],
    ...
    expected_df = pd.DataFrame(
        data={
            "unprocessed_features": ["woofs", "barks", "buzzes"],
            "features": ["woof", "bark", "buzz"],
            "topic": ["dog", "dog", "insect"],
            "use_processed_features": [True, True, False],
        }
    )
    actual_df = backport_sflm_to_sfm.sflm_to_sfm(input_df)
