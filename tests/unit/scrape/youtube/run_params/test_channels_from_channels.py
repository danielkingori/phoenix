...
from phoenix.scrape.youtube.run_params import channels_from_channels
...
def test_get_urls():
    ...
    result = channels_from_channels._get_urls(general_run_params)
    config = general_run_params.run_dt.to_url_config.return_value
    calls = [
        mock.call("static-youtube_channels", config),
        mock.call("source-youtube_channels_from_channels", config),
        mock.call("base-grouped_by_youtube_channels", config),
    ]
    art_url_reg.get_url.assert_has_calls(calls)
    assert isinstance(result, channels_from_channels.ScrapeYouTubeChannelsFromChannelsURLS)
    assert result.config == config
    assert result.static_youtube_channels == art_url_reg.get_url.return_value
    assert result.source_youtube_channels_from_channels == art_url_reg.get_url.return_value
    assert result.base_youtube_channels == art_url_reg.get_url.return_value


@mock.patch("phoenix.scrape.youtube.run_params.channels_from_channels._get_urls")
...
def test_create(m_general_create, m_get_urls):
    ...
    result = channels_from_channels.create(artifact_env, tenant_id, run_datetime_str)
    ...
    assert isinstance(result, channels_from_channels.ScrapeYouTubeChannelsFromChannels)
