...
Version     : v1.1
Description : Extract HTML elements from pages
'''
...
def test_is_file_mbasic(file):
    ...
    if url.netloc == 'mbasic.facebook.com':
        ...
        print(f'File is not a recognized mbasic resource.')
...
class Page(object):
    ...
    def content_not_found(self):
        if 'Content not found' in self.soup.title.text:
            print('File is a "Content not found" mbasic page.')
            return True
    ...
    def create_page(self):
        ...
        self.file_id = generate_random_id()
        ...
        self.top_post_owner = None # the original post creator who instantiated this particular page
    ...
    def get_raw_metadata(self):
        # gets the raw metadata from the html
    ...
    def get_page_metadata(self:BeautifulSoup) -> dict:
        ...
        try:
            ...
            # probably a reaction
            logging.info(f'No "data-ft" attributes. likely reaction or replies with no images - should parse correctly')
            return {}
    ...
    def get_url_components(self, url=None):
        ...
        if '_ft_' in url_params:
            url_params['_ft_'] = ft_parse(url_params) # call sub-function to get dict of '_ft_' element
    ...
    def analyze_path_for_page_type(self):
        ...
        if 'replies' in p_c:
            ...
            # self.soup.find('div', attrs={'id': 'MPhotoActionbar'}).previous_sibling.img for image:
    ...
    def get_parent_post(self):
        ...
        if self.page_type == 'story':
            if 'story.php' not in path:
                # this has a reliable where path = ['watchthemed.alarmphone', 'posts', '2685185111755715']
    ...
    def get_page_id(self):
        ...
        try:
            ...
            if not page_id:
                if 'View post' in self.soup.header.a.text:
                    ...
                    # found in View post back link --> '/story.php?story_fbid=616103579290619&id=314842512750062'
                    ...
                    if 'id' in link_features_query.keys():
                        ...
                        # This might be a back-link on a photo post with a weird url --> fragment='comment_form_148950081810015_840811325957217'
            ...
            logging.info(f'Attribute error in get_page_id(); '
                         f'page may be photo post without strong url query')
        ...
        if self.page_type == 'replies' and page_id == 'id':
            print('Edge case with not easily picking up an ID in replies page.')
    ...
    def get_reaction_page_id(self):
        ...
        if hasattr(self.url_components['query'], '_ft_', ):
    ...
    def parse_react_int(number):
        ...
        # solution from: https://stackoverflow.com/questions/59507827/converting-a-string-object-value-with-m-and-k-to-million-and-thousand
        number = number.strip()
        if number[-1:] == 'K':  # Check if the last digit is K
            converted_number = int(float(number[:-1]) * 1000)  # Remove the last digit with [:-1], and convert to int and multiply by 1000
        else:
            # it's just a number
    ...
    def get_comment_section(self, post_metadata={}):
        # ===========================================================================
        #     Get section of HTML with comments
        #
        # Args:
    ...
    def get_comments(self):
        ...
        except TypeError:  # should know type of page here instead of try/except? see metadata and other markers
            pass
        ...
        if comments_list:
            ...
            self.comments.append(Comment(comment=self.metadata_raw,
                                         comment_parent=comment_parent,
                                         top_level_post_id=self.parent_post, #self.metadata['top_level_post_id'],
                                         top_post=True,
                                         soup=self.soup,
    ...
    def get_subcomments(self):
        # ===========================================================================
        # isolate the comments sections for subcomments and send
        # to Comment object creator
        # ===========================================================================
        ...
        # Of course, the first comment on the page is the top comment but not a post, so set its parent == top_level_post_id
...
class Comment(object):
    ...
    def __init__(self,
        ...
        # very occasionally facebook will preview a reply in the comments and it will mess up the parser
        # if this happens, we don't want the comment.
        # self.reply_preview = self.evaluate_as_reply_preview()

        if top_post == True and page_type == 'photos':
        # special exception if it's a photo post
            ...
            if top_post == False: # and self.reply_preview == False:
            ...
            elif top_post == True:  # top post
                logging.debug('top post')
                self.text = comment[1].text
                self.fb_id = comment_parent  # Top post parent is same as fb_id, because we have page object attached
                self.display_name = self.get_top_post_display_name(comment)
                self.username = self.get_top_post_username(comment)
                self.date = self.get_comment_date(comment[0])  # comment will be is a result set list
    ...
    def get_comment_owner(comment):
        ...
        if url_components[-1][0] == '?':  # 'https://mbasic.facebook.com/richardamdenganvideoeditor/?refid=18&__tn__=R'
            username = url_components[-2]
        else:  # 'https://mbasic.facebook.com/erkumag?refid=18&__tn__=R'
            username = url_components[-1].split('?')[0]

        if username == 'profile.php':  # https://mbasic.facebook.com/profile.php?id=100001749670760&amp;rc=p&amp;__tn__=R
    ...
    def get_photo_post_text_and_owner(self, comment):
        ...
        ## !! This might need debugging because it only accounts for "path='/GovernorMikeSonkoMbuvi/'"
    ...
    def clean_reactions(react_string):
        ...
        # alternative format 'Lowai Hazam and 13K others left reactions, including Like, Love and Care'
        # with arabic can be like this: '\u200e\u200eاسامة شلوف\u200e and 9.6K others\u200e...
        ...
        if imgs:
            for img in imgs:
                ...
                if src_slice == 'png' and 'height' in img.attrs:
                    ...
                    images.append(img.attrs)  # , img) # --> can append full <img> tag for direct web display
        return images


    def get_connections(self, comment, top_post=False):
        ...
        try:  # first search in the comment needs to look under the h3, because h3 has link to post owner
            ...
            try:
                ...
                pass
        if top_post == True:  # It's a top-post element and doesn't have h3 pieces
            links = []
        ...
        for link in links:
            ...
            if href_parse.netloc == 'mbasic.facebook.com':
                ...
                if 'hashtag' in path_components:
                    ...
                    connections['hashtags'].append({'hashtag_url': ht,
                ...
                for key in query:
                    if 'http://' in query[key][0] or 'https://' in query[key][0]:
                        ...
                        conn = {'text': link.text.replace('\n', ''),
                                'link': query[key][0].split('fbclid')[0],} # get rid of fb tracking links
...
def extract_profile_id_from_link(url):
    # handles this exceptional link: https://mbasic.facebook.com/profile.php?id=100035504860378
