"""Finalise run params for youtube videos."""
...
def _get_urls(
    ...
    if not final_url:
        final_url = art_url_reg.get_url("final-youtube_videos", url_config)
    return dtypes.FinaliseRunParamsURLs(
        config=url_config,
        input_dataset=art_url_reg.get_url("tagging_runs-youtube_videos_pulled", url_config),
        ...
        tagging_final=art_url_reg.get_url("tagging_runs-youtube_videos_final", url_config),
        final=final_url,
    )
