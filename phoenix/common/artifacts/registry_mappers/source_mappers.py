...
MAPPERS: MapperDict = {
    ...
    "source-facebook_posts": partial(
        url_mapper, f"{SOURCE_BASE}" + "source-facebook_posts-{RUN_DATETIME}.json"
    ),
    "source-facebook_feed": partial(
        url_mapper, f"{SOURCE_BASE}" + "source-facebook_feed-{RUN_DATETIME}.json"
    ),
    "source-youtube_channels_from_channel_ids": partial(
        url_mapper,
        f"{SOURCE_BASE}" + "source-youtube_channels_from_channel_ids-{RUN_DATETIME}.json",
    ),
    "source-youtube_search_videos_from_channel_ids": partial(
        url_mapper,
        f"{SOURCE_BASE}" + "source-youtube_search_videos_from_channel_ids-{RUN_DATETIME}.json",
    ),
    "source-youtube_comment_threads_from_channel_ids": partial(
        url_mapper,
